...
export default function Page({ params }: { params: { slug: string } }) {
  ...
  const fetchEmployee = async (): Promise<any[]> => {
    try {
      const response = await axios.get(`${apiUrl}/employees/getEmployeeById/${params.slug}`);
      return response.data
  ...
  useEffect(() => {
    const fetchBoth = async () => {
      ...
      const mapDateTempLoadedEmployee = tempLoadedEmployee.map(data => {
        data.date_of_birth = convertDate(data.date_of_birth)
        data.id_card_expiration_date = convertDate(data.id_card_expiration_date)
        return data
      })
      setEmployee(mapDateTempLoadedEmployee);
  ...
  const handleSubmit = async (e: EmployeeData) => {
    try {
      ...
      const response = await axios.patch(`${apiUrl}/employees/updateEmployee/${params.slug}`, e);
    ...
    <div>
      {isLoading ? ("Loading ...") :
        <div className="container mx-auto p-4">
          <h1 className="text-2xl font-bold mb-4">แก้ไขข้อมูลพนักงาน</h1>
          <FormCreate gender={useGender} tambon={useTambon} amphur={useAmphur} province={useProvince} onSubmit={(data: EmployeeData) => handleSubmit(data)} datas={employee} />
        </div>
      }
    </div>
  );
